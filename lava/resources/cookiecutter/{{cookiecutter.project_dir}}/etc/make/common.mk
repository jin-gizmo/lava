...
#  Deploy lava objects in DynamoDB specs. Drop keys starting with $ (e.g. $schema).
#  Usage: $(call lava_put_job,realm,json-file...)
#         json-file can be a space separated list
lava_put_job=dyn-put-item --not-key '$$*' --table lava.$(1).jobs $(2)
lava_put_conn=dyn-put-item --not-key '$$*' --table lava.$(1).connections $(2)
lava_put_trigger=dyn-put-item --not-key '$$*' --table lava.$(1).s3triggers $(2)
...
# Backup the current directory into a zip file in the parent dir.
backup_cwd=( \
		set -e ; \
		dir=$$(basename $$(pwd)) ; \
		bkp="$$dir-$$(date -Iseconds).zip" ; \
		cd .. ; \
		zip -q -r "$$bkp" "$$dir" ; \
		echo "Created a backup to $$bkp" ; \
	)

# Set / update the schema entry in YAML files in current directory.
# Usage: $(call set_schema record-type)
# 	record-type is one of job, connection, s3trigger, event-rule
set_schema=find . -name '*.yaml' | xargs schema-set "$(SCHEMA_BASE)/$(1).schema.yaml"
