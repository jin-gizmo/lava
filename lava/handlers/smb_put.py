"""Put a file onto an SMB file share."""
...
JOB_PARAMS_OPTIONAL_FIELDS = SMB_JOB_PARAMS_OPTIONAL_FIELDS | {'create_dirs'}
...
def run(
    ...
    - path
        Target path within the remote file share. This will be jinja rendered.
    ...
    if not src_file.startswith('s3://') and not os.path.isabs(src_file):
        if not os.path.isabs(basedir):
            basedir = os.path.abspath(basedir)

        src_file = os.path.join(basedir, src_file)

    # ----------------------------------------
    # Put file in SMB using Lava SMB CLI
    ...
    if parameters.get('create_dirs', False):
        cli_cmd.append('--mkdir')
    ...
    return {
        'exit_status': 0,
        'source': src_file,
        'destination': f'{parameters["share_name"]}:{path}',
    }
