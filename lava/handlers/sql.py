...
OUTPUT_PARAM_RE = re.compile(r'^\w+$')
...
def run(
    ...
    try:
        ...
        for payload_file in get_payload_from_s3(
            ...
            payload_base = os.path.basename(payload_file)
            # Get ready for a list of output files.
            return_info['output'][payload_base] = []

            with open(payload_file) as fp:
                sql_text = fp.read()

            LOG.debug('Raw SQL: %s: %s', payload_base, sql_text)
            ...
            for n, sql in enumerate(sql_statements):
                ...
                if cursor.description:
                    outfile_base = f'{payload_base}.{n}{config("SQL_OUTPUT_SUFFIX")}'
                    ...
                    LOG.debug('Data uploaded to %s', s3file)
                    return_info['output'][payload_base].append(s3file)

                LOG.debug('Completed SQL %d: %s', n, sql)
