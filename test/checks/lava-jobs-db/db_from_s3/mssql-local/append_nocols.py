"""Assertions for an MSSQL db_from_s3 append-nocols job."""
...
class Checker(HandlerChecker):
    ...
    def setup(self) -> None:
        ...
        self.cursor.execute(
            f'SELECT * INTO {self.schema}.{self.test_table} ' f'FROM {self.schema}.{self.table}'
        )
    ...
    def check(
        ...
        assert (
            new_row_count == self.row_count + data_rows
        ), 'new_row_count != data_rows + old_row_count'
