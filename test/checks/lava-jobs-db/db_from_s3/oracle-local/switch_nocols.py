"""Assertions for an Oracle db_from_s3 switch job."""
...
class Checker(HandlerChecker):
    ...
    def setup(self) -> None:
        ...
        self.cursor.execute(
            f'CREATE TABLE {self.schema}.{self.test_table}_a '
            f'AS SELECT * FROM {self.schema}.{self.table}'
        )
        self.cursor.execute(
            f'CREATE TABLE {self.schema}.{self.test_table}_b '
            f'AS SELECT * FROM {self.schema}.{self.table} WHERE 1=0'
        )
    ...
    def teardown(self) -> None:
        ...
        if self.conn:
            ...
            for suffix in ('_a', '_b'):
                cursor.execute(f'DROP TABLE IF EXISTS {self.schema}.{self.test_table}{suffix}')
            self.conn.close()
    ...
    def check(
        ...
        self.cursor.execute(f'SELECT COUNT(*) FROM {self.schema}.{self.test_table}_a')
        row_count_a = self.cursor.fetchone()[0]
        assert row_count_a == 0, "Expected table A to be empty but it's not"

        self.cursor.execute(f'SELECT COUNT(*) FROM {self.schema}.{self.test_table}_b')
