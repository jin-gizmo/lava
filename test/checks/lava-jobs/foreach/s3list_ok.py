"""Checker for the s3list-ok foreach job."""
...
from lava.lib.aws import s3_list
...
class Checker(HandlerChecker):
    ...
    def check(
        ...
        """Check function for lava-jobs/foreach/s3list-ok job."""

        expected_s3_objects = set(
            s3_list(
                bucket=self.job_spec['parameters']['foreach']['bucket'],
                prefix=self.job_spec['parameters']['foreach']['prefix'],
                match=self.job_spec['parameters']['foreach'].get('glob'),
            )
        )
        s3_count = len(expected_s3_objects)
        ...
        assert job_result['foreach_len'] == s3_count, 'foreach_len'
        assert job_result['jobs_completed'] == s3_count, 'jobs_completed'
        ...
        assert len(state_items) == s3_count, 'state items count'
        s3_objects = set()
        foreach_indices = set()
        ...
        for item in state_items:
            ...
            s3_objects.add(child_globals['s3obj']['Key'])
        ...
        # Check we got the right s3 objects in our state entries.

        assert expected_s3_objects == s3_objects, 's3 objects'
        ...
        assert len(child_completed_events) == s3_count, 'Child jobs event count'
