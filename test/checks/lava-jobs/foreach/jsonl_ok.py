"""Checker for the jsonl/ok foreach job."""
...
class Checker(HandlerChecker):
    ...
    def check(
        ...
        """Check function for lava-jobs/foreach/jsonl-ok job."""
        ...
        # Get the JSONL source data driving the foreach.
        # This filename will be fully jinja rendered by this point
        foreach_file = self.job_spec['parameters']['foreach']['filename']
        foreach_data = [
            json.loads(line) for line in S3path(foreach_file).read().decode('utf8').splitlines()
        ]
        ...
        assert job_run_info['foreach_len'] == len(foreach_data), 'foreach_len'
        assert job_run_info['jobs_completed'] == len(foreach_data), 'jobs_completed'
        ...
        assert len(child_completed_events) == len(foreach_data), 'Child jobs event count'
