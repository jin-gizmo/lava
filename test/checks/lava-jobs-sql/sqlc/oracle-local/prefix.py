"""Assertions for sqlc/oracle-local/prefix."""
...
class Checker(HandlerChecker):
    ...
    def check(
        ...
        """Validate sqlc output for payload prefix execution."""
        ...
        payloads = {item['payload'] for item in output}
        assert payloads == {'sql1.sql', 'sql2.sql'}

        for item in output:
            assert item['exit_status'] == 0
            stdout_path = item.get('stdout')
            assert stdout_path
