...
#### Installing Docker Payloads

The `docker->registry` key in the environment configuration file controls where
docker payloads are installed. 

If `docker->registry` is missing or an empty string, payload docker images are
deployed to the target AWS account ECR. The install process will create an
appropriate ECR repo and push the image. The uninstall process will delete the
ECR repo. Login to ECR is handled automatically as part of the process.

Alternatively, `docker->registry` cam be set to a different Docker registry.
Payload docker images will be tagged and pushed to the specified registry. The
lava job framework will *not* handle authentication to the registry. That must
be done by the user prior to doing the `make install`. The uninstall process
will not attempt to remove payload images in external registries.

A docker payload in the directory `lava-payloads/plugh.docker` can be
referenced in the job specification like so:

```yaml
job_id: my-docker-job
type: docker
payload: "<{ docker.registry or lava.aws.ecr_uri }>/<{ prefix.docker }>/plugh
```
